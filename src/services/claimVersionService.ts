...
export class ClaimVersionService {
  ...
  async revertToVersion(params: {
    ...
    try {
      ...
      // 获取目标版本数据
      const targetVersionData = await this.getVersionData(
        claimId,
        targetVersion,
      );
  ...
  async getVersionStatistics(claimId: string | RecordId): Promise<{
    ...
    try {
      ...
      const [baseResults, typeResults] = await Promise.all([
        queryWithAuth(this.client, query, { claim_id: claimId }) as Promise<
          unknown[][]
        >,
        queryWithAuth(this.client, typeQuery, { claim_id: claimId }) as Promise<
          unknown[][]
        >,
      ]);
      ...
      const versionsByType: Record<VersionType, number> = {} as Record<
        VersionType,
        number
      >;
   ...
   * 获取当前债权数据
   */
  private async getCurrentClaimData(
    claimId: string | RecordId,
  ...
  private async generateChecksum(
    ...
    try {
      ...
      return this.simpleStringHash(
        JSON.stringify(this.normalizeDataForChecksum(data)),
      );
    }
  }
  ...
  private generateChangeSummary(changes: FieldChange[]): string {
    ...
    if (addedFields.length > 0) {
      summaryParts.push(
        `新增字段: ${addedFields.map((f) => f.field_name).join(", ")}`,
      );
    }

    if (removedFields.length > 0) {
      summaryParts.push(
        `删除字段: ${removedFields.map((f) => f.field_name).join(", ")}`,
      );
    }

    if (modifiedFields.length > 0) {
      summaryParts.push(
        `修改字段: ${modifiedFields.map((f) => f.field_name).join(", ")}`,
      );
    }
