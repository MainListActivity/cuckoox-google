...
class ClaimTrackingAuditService {
  ...
  private async getClient(): Promise<SurrealWorkerAPI> {
    if (!this.clientGetter) {
      try {
        ...
        throw new Error('SurrealDB client not available. Ensure ClaimTrackingAuditService is properly initialized.');
      }
    }
   ...
   * 记录审计事件
   */
  async recordAuditEvent(event: Omit<AuditEvent, 'id' | 'created_at' | 'expires_at'>): Promise<void> {
    try {
      ...
      const auditEvent: AuditEvent = {
        ...
        created_at: new Date(),
        expires_at: new Date(Date.now() + this.AUDIT_RETENTION_DAYS * 24 * 60 * 60 * 1000)
      };

      // 插入审计事件
      await queryWithAuth(client, 
        `CREATE claim_audit_event CONTENT $event`,
        { event: auditEvent }
      );
  ...
  async recordPermissionCheck(
    ...
    const eventType = result === 'denied' ? AuditEventType.PERMISSION_DENIED : AuditEventType.ACCESS;
   ...
   * 处理严重风险
   */
  private async handleCriticalRisk(result: AnomalyDetectionResult): Promise<void> {
    // 立即通知管理员
    // 可以考虑临时锁定用户账户
   ...
   * 处理中等风险
   */
  private async handleMediumRisk(result: AnomalyDetectionResult): Promise<void> {
    // 记录到安全日志
  ...
  private calculateRiskLevel(
    ...
    // 根据资源类型和操作判断风险级别
    if (resourceType === 'claim' && (action === 'export' || action === 'bulk_access')) {
      return RiskLevel.MEDIUM;
    }
  ...
  async queryAuditEvents(filters: {
    ...
    try {
      ...
      const [events] = await queryWithAuth<AuditEvent[][]>(client, query, params);
      return events || [];
  ...
  async getAuditStatistics(filters: {
    ...
    try {
      ...
      const results = await Promise.all(
        queries.map(query => 
          queryWithAuth<any[]>(client, query, params).then(([result]) => result)
        )
      );
  ...
  async cleanupExpiredAuditLogs(): Promise<number> {
    try {
      ...
      const [result] = await queryWithAuth<any[]>(client, query, { now: new Date() });
