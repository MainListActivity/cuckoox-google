...
export class ClaimAuditService {
   ...
   * 记录失败的访问
   */
  private async logFailedAccess(params: LogAccessParams, error: Error): Promise<void> {
    try {
      const clientInfo = this.getClientInfo();
   ...
   * 获取审计日志
   */
  async getAuditLog(options: AuditLogQueryOptions = {}): Promise<ClaimAccessLog[]> {
    try {
      const {
        claim_id,
        user_id,
      ...
      // 添加日期范围过滤
      if (date_range) {
        whereClause += ' AND access_time >= $start_date AND access_time <= $end_date';
  ...
  async getUserAccessStatistics(params: {
    ...
    try {
      ...
      const [baseResults, typeResults, resultResults, mostAccessedResults] = await Promise.all([
      ...
      // 构建统计结果
      const accessesByType: Record<AccessType, number> = {} as Record<AccessType, number>;
      typeStats.forEach((stat: any) => {
        accessesByType[stat.access_type as AccessType] = stat.count;
      });

      const accessesByResult: Record<AccessResult, number> = {} as Record<AccessResult, number>;
   ...
   * 检测异常访问模式
   */
  async detectAnomalousAccess(params: {
    ...
    try {
      ...
      recentAccess.forEach((access: any) => {
        ...
        // 计算异常分数
        const anomalyScore = normalAverage > 0 ? recentCount / normalAverage : recentCount;
        ...
        if (anomalyScore >= threshold_multiplier) {
          ...
          if (access.unique_claims > 20) {
            suspiciousActivities.push(`访问了${access.unique_claims}个不同债权`);
          }
          
          const accessTypes = access.access_types || [];
          if (accessTypes.includes(AccessType.EXPORT) || accessTypes.includes(AccessType.DOWNLOAD)) {
   ...
   * 获取过滤后的审计数据
   */
  private async getFilteredAuditData(filters: AuditFilters): Promise<ClaimAccessLog[]> {
   ...
   * 生成Excel报告
   */
  private async generateExcelReport(data: ClaimAccessLog[], filters: AuditFilters): Promise<string> {
    // 这里应该实现实际的Excel生成逻辑
    // 目前返回模拟的URL
   ...
   * 生成PDF报告
   */
  private async generatePDFReport(data: ClaimAccessLog[], filters: AuditFilters): Promise<string> {
    // 这里应该实现实际的PDF生成逻辑
    // 目前返回模拟的URL
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
