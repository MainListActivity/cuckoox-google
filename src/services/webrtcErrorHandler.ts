...
class WebRTCErrorHandler {
  ...
  handleError(
    ...
    if (this.config.autoRetryEnabled && errorDetails.retryable) {
      ...
      this.listeners.onUserActionRequired?.(errorDetails, errorDetails.suggestedActions || []);
    }
    ...
    switch (error.name) {
      case 'NotAllowedError':
        return {
          ...
          details: {
            userMessage: '您拒绝了摄像头或麦克风访问权限，请在浏览器设置中允许访问',
      ...
      case 'NotReadableError':
        return {
          ...
          details: {
            userMessage: '摄像头或麦克风设备被其他应用程序占用，请关闭其他应用后重试',
   ...
   * 通过错误消息进行分类
   */
  private classifyByMessage(message: string, code: string | number): {
    type: WebRTCErrorType;
    userMessage: string;
    severity: ErrorSeverity;
    recoveryStrategy: RecoveryStrategy;
    ...
    // 媒体相关错误
    if (lowerMessage.includes('getUserMedia') || lowerMessage.includes('media')) {
      return {
        type: WebRTCErrorType.MEDIA_ACCESS_DENIED,
    ...
    if (lowerMessage.includes('not supported') || lowerMessage.includes('unsupported')) {
      return {
        ...
        userMessage: '您的浏览器不支持此功能，请使用最新版本的Chrome、Firefox或Safari',
        severity: ErrorSeverity.CRITICAL,
        recoveryStrategy: RecoveryStrategy.USER_ACTION,
        retryable: false,
        suggestedActions: [
  ...
  private generateTechnicalDetails(
    ...
    details.push(`getUserMedia Supported: ${!!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia)}`);
  ...
  private attemptRecovery(error: WebRTCErrorDetails): void {
    const maxRetries = error.maxRetries || this.getMaxRetriesForError(error.type);
    ...
    const retryDelay = error.retryDelay || this.getRetryDelayForError(error.type);
  ...
  markErrorResolved(
    ...
    if (startTime) {
      ...
      // 更新平均解决时间
      const totalTime = this.errorStatistics.averageResolutionTime * (this.errorStatistics.totalErrors - 1);
      this.errorStatistics.averageResolutionTime = (totalTime + resolutionTime) / this.errorStatistics.totalErrors;
      
      this.resolutionStartTimes.delete(errorId);
    }

    // 查找对应的错误并触发解决事件
    const error = this.errorHistory.find(err => err.context?.errorId === errorId);
    if (error) {
