...
class BusinessNotificationService {
  ...
  private async getClient(): Promise<SurrealWorkerAPI> {
    if (!this.clientGetter) {
      throw new Error(
        "SurrealDB client not available. Ensure BusinessNotificationService is properly initialized with setClientGetter.",
      );
    }
  ...
  async notifyClaimReviewed(
    ...
    try {
      ...
      // Get claim details
      const claim = await queryWithAuth<ClaimData[]>(
        client,
      ...
      console.log(
        `Sent claim review notification for claim ${claimData.claim_number}`,
      );
    } catch (error) {
  ...
  async notifyMeetingChange(
    ...
    try {
      ...
      const meetingTime = new Date(
        meetingData.scheduled_start_date,
      ...
      if (meetingData.participants && meetingData.participants.length > 0) {
        ...
        console.log(
          `Sent meeting ${changeType} notifications to ${meetingData.participants.length} participants`,
        );
      }

      // Also send to all case members
      const [caseMembers] = await client.query<
        Array<{ user_id: RecordId | string }>[]
        ...
        for (const member of caseMembers) {
          ...
          if (
            ...
            !meetingData.participants.some(
              (p: RecordId | string) => String(p) === String(member.user_id),
            )
          ) {
            await messageService.sendNotification({
              type: "BUSINESS_NOTIFICATION",
  ...
  async notifyCaseStatusChange(
    ...
    try {
      ...
      if (caseMembers && Array.isArray(caseMembers) && caseMembers.length > 0) {
        ...
        console.log(
          `Sent case status change notifications to ${caseMembers.length} members`,
        );
      }
    } catch (error) {
  ...
  async sendSystemAnnouncement(
    ...
    try {
      ...
      if (targetUserIds && targetUserIds.length > 0) {
        ...
        console.log(
          `Sent system announcement to ${targetUserIds.length} users`,
        );
      } else {
  ...
  async notifyCreditorAdded(
    ...
    try {
      ...
      const creditorResult = await client.query<
        Array<{ name: string; legal_id: string }>[]
      ...
      if (
        !Array.isArray(creditorResult) ||
        creditorResult.length === 0 ||
        !Array.isArray(creditorResult[0]) ||
        creditorResult[0].length === 0 ||
        !Array.isArray(caseResult) ||
        caseResult.length === 0 ||
        !Array.isArray(caseResult[0]) ||
        caseResult[0].length === 0
      ) {
      ...
      const content =
        `案件 ${caseInfo.case_number} 新增债权人：\n\n` +
        `债权人名称：${creditorInfo.name}\n` +
        `证件号码：${creditorInfo.legal_id}\n\n` +
        `请及时关注债权申报情况。`;

      // Notify case lead
      const caseLeadResult = await client.query<
        Array<{ case_lead_user_id: RecordId | string }>[]
      >(`SELECT case_lead_user_id FROM ${String(caseId)}`);

      if (
        Array.isArray(caseLeadResult) &&
        caseLeadResult.length > 0 &&
        Array.isArray(caseLeadResult[0]) &&
        caseLeadResult[0].length > 0 &&
  ...
  async notifyClaimOperation(
    ...
    try {
      ...
      >(`SELECT display_name FROM user WHERE id = $operatorId`, { operatorId });
      ...
      if (["submit"].includes(operationType)) {
        ...
        ["review", "approve", "reject", "supplement_request"].includes(
          operationType,
        )
      ) {
        // 审核类操作：通知债权人
      ...
      for (const recipientId of recipients) {
        if (String(recipientId) !== String(operatorId)) {
          // 不通知操作者自己
          await messageService.sendNotification({
            type: "BUSINESS_NOTIFICATION",
      ...
      console.log(
        `Sent claim operation notification for operation ${operationType} on claim ${claimData.claim_number}`,
      );
    } catch (error) {
  ...
  async notifyClaimStatusChange(
    ...
    try {
      ...
      console.log(
        `Sent claim status change notification from ${fromStatus} to ${toStatus} for claim ${claimData.claim_number}`,
      );
    } catch (error) {
  ...
  async notifyClaimReviewTimeout(
    ...
    try {
      ...
      const content =
        `以下债权申报已超过${timeoutDays}天未审核，请尽快处理：\n\n` +
        `债权编号：${claimData.claim_number}\n` +
      ...
      if (caseMembers && caseMembers.length > 0) {
        ...
        console.log(
          `Sent timeout reminder for claim ${claimData.claim_number} to ${caseMembers.length} reviewers`,
        );
      }
    } catch (error) {
  ...
  async notifyClaimVersionUpdate(
    ...
    try {
      ...
      for (const recipientId of uniqueRecipients) {
        if (!changedBy || String(recipientId) !== String(changedBy)) {
          // 不通知操作者自己
          await messageService.sendNotification({
            type: "BUSINESS_NOTIFICATION",
            target_user_id: recipientId,
      ...
      console.log(
        `Sent claim version update notification for claim ${claimData.claim_number} version ${versionNumber}`,
      );
    } catch (error) {
  ...
  async sendBatchTimeoutReminders(timeoutDays: number = 7) {
    try {
      ...
      const [timeoutClaims] = await client.query<
        Array<{
          id: RecordId | string;
          claim_number: string;
          created_at: string;
      ...
      if (timeoutClaims && timeoutClaims.length > 0) {
        console.log(
          `Found ${timeoutClaims.length} timeout claims, sending reminders...`,
        );

        for (const claim of timeoutClaims) {
          await this.notifyClaimReviewTimeout(
            claim.id,
            claim.created_at,
            timeoutDays,
          );
        ...
        console.log(
          `Sent timeout reminders for ${timeoutClaims.length} claims`,
        );
      }
    } catch (error) {
