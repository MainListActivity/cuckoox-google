...
export interface CreateGroupData {
  ...
  settings?: Partial<Omit<GroupSettings, 'group_id' | 'created_at' | 'updated_at'>>;
}
...
class GroupManager {
  ...
  private async getClient(): Promise<SurrealWorkerAPI> {
    if (!this.clientGetter) {
      try {
        ...
        throw new Error('SurrealDB client not available. Ensure GroupManager is properly initialized with setClientGetter.');
      }
    }
   ...
   * 创建群组
   */
  async createGroup(data: CreateGroupData): Promise<{ group: Group; settings: GroupSettings; members: GroupMember[] }> {
    try {
      ...
      const groupData: Omit<Group, 'id'> = {
        ...
        case_id: data.case_id ? 
          (typeof data.case_id === 'string' ? new RecordId('case', data.case_id.split(':')[1]) : data.case_id) : undefined,
      ...
      const settingsData: Omit<GroupSettings, 'id'> = {
        ...
        member_join_notification: data.settings?.member_join_notification ?? true,
        member_leave_notification: data.settings?.member_leave_notification ?? true,
        created_at: now,
        updated_at: now
      };
      ...
      const [ownerMember] = await client.create('group_member', ownerMemberData);
      ...
      if (data.initial_members && data.initial_members.length > 0) {
        for (const userId of data.initial_members) {
          ...
          const memberData: Omit<GroupMember, 'id'> = {
            group_id: group.id,
            user_id: typeof userId === 'string' ? new RecordId('user', userId.split(':')[1]) : userId,
            role: 'member',
            joined_at: now,
            invited_by: '$auth.id',
   ...
   * 更新群组信息
   */
  async updateGroup(groupId: RecordId | string, data: UpdateGroupData): Promise<Group> {
    try {
      const client = await this.getClient();
  ...
  async deleteGroup(groupId: RecordId | string): Promise<boolean> {
    try {
      ...
      const groupRecordId = typeof groupId === 'string' ? 
        new RecordId('group', groupId.split(':')[1]) : groupId;

      // 删除相关数据
      await Promise.all([
        // 删除群组成员
        client.query('DELETE group_member WHERE group_id = $group_id', { group_id: groupRecordId }),
        // 删除群组设置
        client.query('DELETE group_settings WHERE group_id = $group_id', { group_id: groupRecordId }),
        // 删除群组邀请
        client.query('DELETE group_invitation WHERE group_id = $group_id', { group_id: groupRecordId }),
        // 删除群组消息 (可选，根据业务需求)
   ...
   * 添加成员到群组
   */
  async addMembers(groupId: RecordId | string, data: AddMemberData): Promise<GroupMember[]> {
    try {
      const client = await this.getClient();
      ...
      // 检查群组是否存在且获取设置
      const [group] = await client.query(`SELECT * FROM ${typeof groupId === 'string' ? groupId : String(groupId)}`);
      if (!group) {
      ...
      const [countResult] = await client.query(memberCountQuery, { 
        group_id: typeof groupId === 'string' ? new RecordId('group', groupId.split(':')[1]) : groupId 
      });
      
      if (group.max_members && countResult.total + data.user_ids.length > group.max_members) {
   ...
   * 从群组移除成员
   */
  async removeMember(groupId: RecordId | string, userId: RecordId | string): Promise<boolean> {
    try {
      const client = await this.getClient();
   ...
   * 更新成员角色
   */
  async updateMemberRole(groupId: RecordId | string, data: UpdateMemberRoleData): Promise<GroupMember> {
    try {
      const client = await this.getClient();
      ...
      const [member] = await client.query(memberQuery, {
        group_id: typeof groupId === 'string' ? new RecordId('group', groupId.split(':')[1]) : groupId,
        user_id: typeof data.user_id === 'string' ? new RecordId('user', data.user_id.split(':')[1]) : data.user_id
      });
   ...
   * 转让群主
   */
  async transferOwnership(groupId: RecordId | string, newOwnerId: RecordId | string): Promise<boolean> {
    try {
      const client = await this.getClient();
      ...
      const [newOwnerMember] = await client.query(memberQuery, {
        group_id: typeof groupId === 'string' ? new RecordId('group', groupId.split(':')[1]) : groupId,
        new_owner_id: typeof newOwnerId === 'string' ? new RecordId('user', newOwnerId.split(':')[1]) : newOwnerId
      });
  ...
  async getUserGroups(userId?: RecordId | string): Promise<Group[]> {
    try {
      ...
      const query = `
        ...
        JOIN group_member ON group_member.group_id = message_group.id 
   ...
   * 检查用户权限
   */
  private async checkPermission(groupId: RecordId | string, permission: keyof GroupMember['permissions']): Promise<void> {
    const client = await this.getClient();
   ...
   * 检查管理员权限
   */
  private async checkAdminPermission(groupId: RecordId | string): Promise<void> {
    const client = await this.getClient();
   ...
   * 检查群主权限
   */
  private async checkOwnerPermission(groupId: RecordId | string): Promise<void> {
    const client = await this.getClient();
   ...
   * 根据角色获取默认权限
   */
  private getDefaultPermissionsByRole(role: GroupMemberRole): GroupMember['permissions'] {
    switch (role) {
      case 'owner':
        return {
          can_send_message: true,
