 ...
 * 获取所有可用角色
 */
export const getAllRoles = async (client: Surreal): Promise<Role[]> => {
 ...
 * 根据ID获取单个角色
 */
export const getRoleById = async (client: Surreal, roleId: RecordId): Promise<Role | null> => {
  console.log(`[RoleService] Fetching role by ID: ${roleId}`);
  
  try {
    const result = await client.select<Role>(roleId);
 ...
 * 根据名称获取角色
 */
export const getRoleByName = async (client: Surreal, roleName: string): Promise<Role | null> => {
 ...
 */
export const getCaseMemberRoles = async (client: Surreal): Promise<Role[]> => {
