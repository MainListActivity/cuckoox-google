...
class ClaimStatisticsService {
  ...
  private async getClient(): Promise<SurrealWorkerAPI> {
    if (!this.clientGetter) {
      try {
        ...
        throw new Error('SurrealDB client not available. Ensure ClaimStatisticsService is properly initialized.');
      }
    }
  ...
  async getProcessingEfficiencyStats(
    ...
    try {
      ...
      // 获取处理时长统计
      const [processingStats] = await queryWithAuth<Array<{
        total_claims: number;
      ...
      // 获取时间分布统计
      const [timeRanges] = await queryWithAuth<Array<{
        range: string;
        count: number;
      }>[]>(client,
        `SELECT 
  ...
  async getQualityIndicatorStats(
    ...
    try {
      ...
      // 获取质量指标
      const [qualityStats] = await queryWithAuth<Array<{
      ...
      return {
        ...
        rejectionRate: Math.round((stats.rejection_count / totalReviewed) * 100),
        supplementRequestRate: Math.round((stats.supplement_count / totalReviewed) * 100),
  ...
  async getWorkloadStats(
    ...
    try {
      ...
      if (dateRange) {
        whereClause += ' AND operation_time >= $startDate AND operation_time <= $endDate';
        params.startDate = dateRange.start.toISOString();
        params.endDate = dateRange.end.toISOString();
      }
      
      // 获取审核人员工作量统计
      const [reviewerStats] = await queryWithAuth<Array<{
      ...
      // 获取每日工作量统计
      const [dailyStats] = await queryWithAuth<Array<{
        date: string;
      ...
      return {
        reviewerStats: (reviewerStats || []).map(reviewer => ({
          ...
          efficiency: reviewer.total_reviewed > 0 ? 
            Math.round(100 / (reviewer.avg_processing_time || 1)) : 0
        })),
  ...
  async getStatusFlowStats(
    ...
    try {
      ...
      if (dateRange) {
        whereClause += ' AND transition_time >= $startDate AND transition_time <= $endDate';
        params.startDate = dateRange.start.toISOString();
        params.endDate = dateRange.end.toISOString();
      }
      
      // 获取状态分布
      const [statusDist] = await queryWithAuth<Array<{
        status: string;
        count: number;
      }>[]>(client,
        `SELECT 
      ...
      // 获取流转分析
      const [flowAnalysis] = await queryWithAuth<Array<{
        from_status: string;
        to_status: string;
        count: number;
        avg_duration: number;
      ...
      const totalClaims = statusDist.reduce((sum, item) => sum + item.count, 0) || 1;
  ...
  async getBottleneckAnalysis(
    ...
    try {
      ...
      // 获取慢处理债权
      const [slowClaims] = await queryWithAuth<Array<{
      ...
      // 获取瓶颈阶段分析
      const [bottleneckStages] = await queryWithAuth<Array<{
        stage: string;
        avg_duration: number;
        claim_count: number;
      }>[]>(client,
      ...
      return {
        ...
        bottleneckStages: bottleneckStages.map(stage => ({
          ...
          severity: stage.avg_duration > 14 ? 'high' : 
                   stage.avg_duration > 7 ? 'medium' : 'low'
        }))
      };
  ...
  async getTimeSeriesData(
    ...
    try {
      ...
      const [timeSeriesData] = await queryWithAuth<Array<{
        date: string;
        submissions: number;
        approvals: number;
        rejections: number;
  ...
  async exportStatisticsReport(
    ...
    try {
      const [efficiency, quality, workload, statusFlow, bottleneck, timeSeries] = await Promise.all([
