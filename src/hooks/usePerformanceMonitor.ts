...
import type {
  PerformanceMetrics,
  AppShellState,
} from "../workers/pwa-performance-manager";
...
export const usePerformanceMonitor = (
  ...
  useEffect(() => {
    ...
    if ("PerformanceObserver" in window) {
      ...
      try {
        ...
        console.warn(
          "usePerformanceMonitor: Failed to setup PerformanceObserver:",
          error,
        );
      }
  ...
  const collectBrowserMetrics = useCallback(() => {
    try {
      ...
      console.error(
        "usePerformanceMonitor: Error collecting browser metrics:",
        error,
      );
    }
  }, []);

  const processBrowserPerformanceEntries = useCallback(
  ...
  const loadServiceWorkerData = useCallback(async () => {
    ...
    try {
      await Promise.all([loadPerformanceMetrics(), loadAppShellState()]);
  ...
  const loadPerformanceMetrics = async (): Promise<void> => {
    ...
    try {
      ...
      if (registration.active) {
        ...
        return new Promise((resolve) => {
          const handleMessage = (event: MessageEvent) => {
            if (
              event.data.type === "get_performance_metrics_response" &&
      ...
      console.error(
        "usePerformanceMonitor: Error loading performance metrics:",
        error,
      );
    }
  };
    ...
    try {
      ...
      if (registration.active) {
        ...
        return new Promise((resolve) => {
          const handleMessage = (event: MessageEvent) => {
            if (
              event.data.type === "get_app_shell_state_response" &&
              event.data.messageId === messageId
            ) {
              navigator.serviceWorker.removeEventListener(
                "message",
      ...
      console.error(
        "usePerformanceMonitor: Error loading App Shell state:",
        error,
      );
    }
  };
  ...
  const preloadResources = useCallback(
    async (urls: string[]): Promise<void> => {
      ...
      try {
        ...
        console.error(
          "usePerformanceMonitor: Error preloading resources:",
          error,
        );
        throw error;
      }
  ...
  const forceMemoryCleanup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      console.error(
        "usePerformanceMonitor: Error during memory cleanup:",
        error,
      );
