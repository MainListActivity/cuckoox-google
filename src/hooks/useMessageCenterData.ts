      ...
      if (data) {
        const parsedConversations: ConversationSummary[] = data.map(raw => {
          ...
          const participantsSummary = raw.participants?.map(p => ({
            id: p.id,
            name: p.name || (typeof p.id === 'string' ? p.id.split(':')[1] : 'Unknown User') // Very basic name assumption
          })) || [];

          // Extract last message details safely, considering subquery results might be arrays
          const snippet = typeof raw.last_message_snippet === 'string' ? raw.last_message_snippet : 
                          (Array.isArray(raw.last_message_snippet) && raw.last_message_snippet[0]) ? raw.last_message_snippet[0].content : 'No messages yet';
          const timestamp = typeof raw.last_message_timestamp === 'string' ? raw.last_message_timestamp :
                            (Array.isArray(raw.last_message_timestamp) && raw.last_message_timestamp[0]) ? raw.last_message_timestamp[0].created_at : raw.updated_at || new Date(0).toISOString();
          const senderName = typeof raw.last_message_sender_name === 'string' ? raw.last_message_sender_name :
