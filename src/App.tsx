...
const DocumentCenterDemo = React.lazy(
  () => import("@/src/pages/documents/demo"),
);
...
const CreditorListPage = React.lazy(
  () => import("@/src/pages/creditors/index"),
); // MODIFIED PATH
const ClaimListPage = React.lazy(() => import("@/src/pages/claims/index"));
const ClaimSubmissionPage = React.lazy(
  () => import("@/src/pages/claims/submit"),
);
const ClaimAttachmentPage = React.lazy(
  () => import("@/src/pages/claims/attachment"),
);
const SubmittedClaimDetailPage = React.lazy(
  () => import("@/src/pages/my-claims/[claimId]"),
);
...
const AccessDeniedRolePage = React.lazy(
  () => import("@/src/pages/access-denied-role"),
);
const CaseStatusToggler = React.lazy(
  () => import("@/src/components/admin/CaseStatusToggler"),
); // This is a component, path is likely correct
const ClaimReviewDetailPage = React.lazy(
  () => import("@/src/pages/claims/[claimId]/review"),
);
...
const AdminPWAMonitorPage = React.lazy(
  () => import("@/src/pages/admin/pwa-monitor"),
);
...
const AdminCreateClaimAttachmentsPage = React.lazy(
  () => import("@/src/pages/admin/create-claim-attachments"),
);
...
const PermissionManagementPage = React.lazy(
  () => import("@/src/pages/admin/manage/permissions"),
);
const CaseMemberManagementPage = React.lazy(
  () => import("@/src/pages/case-members/index"),
);
const RootAdminPage = React.lazy(() => import("@/src/pages/root-admin/index"));
const RootAdminLoginPage = React.lazy(
  () => import("@/src/pages/root-admin/login"),
);
...
function App() {
  ...
  useEffect(() => {
      ...
      if (auth.isLoggedIn && auth.userCases && auth.userCases.length > 0) {
        ...
        if (caseIdFromUrl) {
          if (auth.selectedCaseId !== new RecordId("case", caseIdFromUrl)) {
            console.log(
              `Attempting to select case from URL parameter: ${caseIdFromUrl}`,
            );
            ...
            const isValidCase = auth.userCases.some(
              (c) => c.id.toString() === caseIdFromUrl,
            );
            if (isValidCase) {
              await auth.selectCase(caseIdFromUrl);
            } else {
              console.warn(
                `Case ID ${caseIdFromUrl} from URL is not valid for the current user or userCases not loaded yet.`,
              );
            }
          }
    ...
    location.pathname,
    navigate,
  ]); // Explicitly excluding 'auth' and 'location.state' to prevent unnecessary re-renders
  ...
  if (
    ...
    location.pathname === "/root-admin" ||
    location.pathname === "/root-admin/login"
  ) {
    return (
      <ErrorBoundary>
        <CustomThemeProvider>
          <SnackbarProvider>
            <CaseStatusProvider>
              <Suspense
                fallback={
                  <GlobalLoader
                    message={t("loader.pageLoading", "Loading page...")}
                  />
                }
              >
                <Routes>
                ...
                >
                  <Routes>
                    ...
                    <Route
                      ...
                      element={
                        (
                          <ProtectedRoute requiredCaseStatus="债权申报">
                            <ClaimSubmissionPage />
                          </ProtectedRoute>
                        ) as ReactNode
                      }
                    ...
                    />
                    <Route
                      path="/my-claims/:claimId/submitted"
                      element={
                        <ProtectedRoute requiredCaseStatus="债权申报">
                          <SubmittedClaimDetailPage />
                        </ProtectedRoute>
                    ...
                    />{" "}
                    {/* Route for creditor viewing their submitted claim */}
                    ...
                    <Route
                      path="/admin/create-claim/:tempClaimId/attachments"
                      element={
                        <ProtectedRoute requiredRole="admin">
                    ...
                    />
                    <Route
                      path="/admin/case-status-toggler"
                      element={
                        (
                          <ProtectedRoute requiredRole="admin">
                            <CaseStatusToggler />
                    ...
                    />
                    <Route
                      path="/access-denied-status"
                      element={<AccessDeniedPage />}
                    />{" "}
                    {/* <-- ADDED */}
                    <Route
                      path="/access-denied-role"
                      element={<AccessDeniedRolePage />}
                    />{" "}
                    {/* <-- ADDED */}
