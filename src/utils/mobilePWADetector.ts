...
export interface InstallationHistoryEntry {
  timestamp: number;
  action: 'prompt_shown' | 'prompt_accepted' | 'prompt_dismissed' | 'manual_guide_viewed';
  platform: string;
  browser: string;
...
class MobilePWADetector {
  ...
  private detectDevice(): DeviceInfo {
    ...
    // PWA安装能力检测
    const canInstallPWA = this.checkPWAInstallCapability(platform, browser, browserVersion);
    const supportsNativeInstall = this.checkNativeInstallSupport(platform, browser);
  ...
  private checkPWAInstallCapability(
    ...
    switch (platform) {
      ...
      case 'android':
        ...
        // Samsung Browser 支持
        if (browser === 'samsung') {
          const samsungVersion = parseInt(version);
  ...
  private checkNativeInstallSupport(
    ...
    if (platform === 'android' && ['chrome', 'samsung'].includes(browser)) return true;
    if (platform === 'desktop' && ['chrome', 'edge'].includes(browser)) return true;
    return false;
  }
   ...
   * 获取设备名称
   */
  private getDeviceName(platform: DeviceInfo['platform'], userAgent: string): string {
    if (platform === 'ios') {
  ...
  private saveInstallState(): void {
    if (this.installState) {
      try {
        localStorage.setItem(this.storageKey, JSON.stringify(this.installState));
  ...
  private recordUserAction(
    ...
    if (this.installState.installationHistory.length > 50) {
      this.installState.installationHistory = this.installState.installationHistory.slice(-30);
    }
  ...
  public shouldShowInstallPrompt(): boolean {
    ...
    // 基本条件检查
    if (state.isInstalled || !device.canInstallPWA || !state.canShowInstallPrompt) {
      return false;
    }
  ...
  private getIOSInstallGuidance(device: DeviceInfo): InstallGuidance {
    ...
    return {
      ...
      warnings: device.osVersion && parseFloat(device.osVersion) < 11.3 
...
export const shouldShowInstallPrompt = () => mobilePWADetector.shouldShowInstallPrompt();
