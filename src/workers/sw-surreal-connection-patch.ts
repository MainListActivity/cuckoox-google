...
import { SimplifiedConnectionManager, ConnectionEventType, ConnectionEvent } from './simplified-connection-manager';
...
export async function initializeSimplifiedConnectionManager(): Promise<void> {
  ...
  try {
    ...
    console.error('🔧 Failed to initialize SimplifiedConnectionManager:', error);
    throw error;
  }
}
...
export async function connectWithSimplifiedManager(config: {
  ...
  console.log('🔧 SimplifiedConnectionManager is available, current state:', simplifiedConnectionManager.getState());
  ...
  try {
    ...
    console.log('🔧 simplifiedConnectionManager.connect completed successfully');
...
export function getSimplifiedConnectionState(): {
  ...
  if (!simplifiedConnectionManager) {
    console.warn('🔧 SimplifiedConnectionManager not initialized - returning fallback state');
    return {
      state: 'ready', // 🔧 改为ready状态，让页面可以正常显示
      isConnected: true, // 🔧 假设连接正常，避免阻塞UI
      isAuthenticated: false,
      hasDb: false,
      isReconnecting: false,
      reconnectAttempts: 0,
      endpoint: null,
      error: null, // 🔧 不报告错误，避免阻塞
      latency: null
    };
  }
