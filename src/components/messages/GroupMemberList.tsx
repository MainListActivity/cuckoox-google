...
import {
  ...
  useGroupOperations,
  useGroupPermissions,
  useGroupWebRTCPermissions,
} from "@/src/hooks/useGroupData";
...
export default function GroupMemberList({
  ...
  // 群组数据
  const { group, members, isLoading, error, refetch } =
    useGroupDetails(groupId);
  const { removeMember, updateMemberRole, transferOwnership } =
    useGroupOperations();
  const permissions = useGroupPermissions(groupId, user?.id);

  // 群组WebRTC权限检查
  const { canInitiateVoiceCall, canInitiateVideoCall } =
  ...
  const memberStats = useMemo(() => {
    if (!members)
      return { total: 0, online: 0, owners: 0, admins: 0, members: 0 };
  ...
  const handleMemberAction = useCallback(
    async (action: string, member: ExtendedGroupMember) => {
      const actions = {
        makeAdmin: async () => {
    ...
    return (
      <ListItem key={member.id} divider>
        <ListItemAvatar>
          ...
          >
            <Avatar src={member.user_avatar} sx={{ width: 48, height: 48 }}>
    ...
    <Box>
      ...
      <Paper elevation={1} sx={{ p: 2, mb: 2 }}>
        <Box display="flex" alignItems="center" gap={2} mb={2}>
          ...
          <Box flex={1}>
            <Typography variant="h6">{group.name} - 成员列表</Typography>
        ...
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid size={3}>
            <Card variant="outlined">
              <CardContent sx={{ textAlign: "center", py: 1 }}>
                <Typography variant="h6">{memberStats.members}</Typography>
