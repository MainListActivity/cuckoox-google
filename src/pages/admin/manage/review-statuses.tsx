...
import AddEditStatusDialog, {
  StatusData,
} from "@/src/pages/components/admin/review_statuses/AddEditStatusDialog";
...
const initialMockReviewStatuses: ReviewStatus[] = [
  {
    ...
    label: "审核通过",
    description:
      "债权已完全审核通过, 无任何疑问或需要补充的材料。符合所有法定及程序要求。",
  },
  {
    id: "s2",
    label: "信息不全驳回",
    description:
      "因提交的核心信息不完整或关键字段缺失而被驳回。债权人需核对并补全信息后重新提交。",
  },
  {
    id: "s3",
    label: "要求补充材料",
    description:
      "当前提交材料不足以支撑债权主张, 需要债权人补充额外证明文件或详细说明。",
  },
  {
    id: "s4",
    label: "待审核",
    description: "债权申报已提交, 等待管理人进行初步审核。此为默认初始状态。",
  },
  {
    id: "s5",
    label: "审核中",
    description: "管理人正在对债权材料进行详细审查与核实。",
  },
  {
    id: "s6",
    label: "部分通过",
    description:
      "债权的部分金额或内容得到认可, 另一部分可能存在争议或未被确认。",
  },
];
...
const ReviewStatusManagementPage: React.FC = () => {
  ...
  const [editingStatusData, setEditingStatusData] = useState<StatusData | null>(
    null,
  );
  ...
  const handleOpenEditStatusDialog = (statusId: string) => {
    const statusToEdit = reviewStatuses.find(
  ...
  const handleConfirmDeleteStatus = useCallback(() => {
    if (deletingStatusId) {
      setReviewStatuses((prevStatuses) =>
        prevStatuses.filter((status) => status.id !== deletingStatusId),
      );
    ...
    deletingStatusId,
    editingStatusData?.id,
    handleCloseStatusDialog,
    handleCloseDeleteConfirm,
  ]);

  // Find the status to delete for the dialog's content text
  const statusToDeleteDetails = reviewStatuses.find(
    (s) => s.id === deletingStatusId,
  );
