...
describe('DataCacheManager Auto Sync', () => {
  ...
  describe('Auto Sync Table Detection', () => {
    it('should correctly identify auto sync tables', () => {
      // Test all tables from the actual AUTO_SYNC_TABLES array
      ...
      expect(isAutoSyncTable('claim_operation_log')).toBe(true);
      expect(isAutoSyncTable('claim_version_history')).toBe(true);
      expect(isAutoSyncTable('claim_status_flow')).toBe(true);
      expect(isAutoSyncTable('claim_access_log')).toBe(true);
    });
