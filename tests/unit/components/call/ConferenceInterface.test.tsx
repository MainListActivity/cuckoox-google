...
const TestConferenceInterface = React.lazy(() => 
  Promise.resolve({
    default: (props: any) => {
      ...
      const [showParticipantMenu, setShowParticipantMenu] = React.useState<string | null>(null);
      ...
      const [sidebarType, setSidebarType] = React.useState<'participants' | 'chat' | null>(null);
      ...
      const handleManageParticipant = async (userId: string, action: string) => {
        ...
        try {
          ...
          switch (action) {
            ...
            case 'unmute':
              ...
              break;
            case 'promote':
              await callManager.setParticipantRole(props.callId, userId, 'moderator');
              break;
            case 'demote':
              await callManager.setParticipantRole(props.callId, userId, 'participant');
              break;
            case 'kick':
      ...
      return React.createElement('div', { 
        ...
        children: [
          ...
          React.createElement('div', { key: 'participants-count' }, `参与者: ${participants.length}人`),
          React.createElement('div', { key: 'conference-id' }, '会议ID: 123456'),
          ...
          React.createElement('div', { key: 'sidebar' }, `侧边栏: ${sidebarType || '无'}`),
            ...
            children: participants.map(p => 
              React.createElement('div', {
                ...
                children: [
                  ...
                  React.createElement('div', { key: 'role' }, `角色: ${p.role}`),
                  React.createElement('div', { key: 'status' }, p.mediaState.micMuted ? '已静音' : '未静音'),
                  React.createElement('div', { key: 'camera' }, p.mediaState.cameraOff ? '摄像头关闭' : '摄像头开启'),
                  !p.isLocal && React.createElement('button', {
                    key: 'manage-btn',
          ...
          // 参与者管理菜单
          showParticipantMenu && React.createElement('div', {
            ...
            children: [
              ...
              React.createElement('button', { 
                ...
                onClick: () => handleManageParticipant(showParticipantMenu, 'mute') 
              }, '静音'),
              React.createElement('button', { 
                key: 'unmute-participant', 
                onClick: () => handleManageParticipant(showParticipantMenu, 'unmute') 
              }, '取消静音'),
              React.createElement('button', { 
                key: 'promote-participant', 
                onClick: () => handleManageParticipant(showParticipantMenu, 'promote') 
              }, '设为管理员'),
              React.createElement('button', { 
                key: 'demote-participant', 
                onClick: () => handleManageParticipant(showParticipantMenu, 'demote') 
              }, '设为参与者'),
              React.createElement('button', { 
                key: 'kick-participant', 
                onClick: () => handleManageParticipant(showParticipantMenu, 'kick') 
              }, '移出会议')
            ]
          }),
          ...
          // 参与者侧边栏
          sidebarType === 'participants' && React.createElement('div', {
            key: 'participants-sidebar',
            'data-testid': 'participants-sidebar',
            children: [
              React.createElement('div', { key: 'sidebar-title' }, '参与者列表'),
              React.createElement('button', { 
                key: 'close-sidebar',
                'aria-label': '关闭侧边栏',
              ...
              ...participants.map(p => 
                React.createElement('div', {
                  ...
                  children: [
                    ...
                    React.createElement('div', { key: 'connection' }, p.connectionState)
                  ]
                })
              )
            ]
          ...
          showEndCallConfirm && React.createElement('div', {
            ...
            children: [
              React.createElement('div', { key: 'title', id: 'dialog-title' }, '结束会议'),
              React.createElement('div', { key: 'content' }, '确定要结束整个会议吗？所有参与者都将被断开连接。'),
              React.createElement('button', { 
                key: 'cancel', 
...
describe('ConferenceInterface', () => {
  ...
  describe('组件渲染', () => {
    ...
    it('应该显示参与者网格', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('participant-participant-1')).toBeInTheDocument();
        expect(screen.getByTestId('participant-participant-2')).toBeInTheDocument();
      });
    });
  ...
  describe('用户交互', () => {
    it('应该能够切换静音状态', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.toggleMute).toHaveBeenCalledWith('test-conference-id');
      });
    });
    ...
    it('应该能够切换摄像头状态', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.toggleCamera).toHaveBeenCalledWith('test-conference-id');
      });
    });
    ...
    it('应该能够开始屏幕共享', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.startScreenShare).toHaveBeenCalledWith('test-conference-id');
      });
    });
    ...
    it('应该能够打开参与者列表', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('参与者列表')).toBeInTheDocument();
    ...
    it('应该能够邀请参与者', async () => {
      ...
      await waitFor(() => {
        expect(defaultProps.onInviteParticipants).toHaveBeenCalledWith('test-conference-id');
      });
    });
    ...
    it('应该能够调整视频质量', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.adjustVideoQuality).toHaveBeenCalledWith('test-conference-id', 'high');
  ...
  describe('参与者管理', () => {
    it('应该能够打开参与者管理菜单', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('管理参与者')).toBeInTheDocument();
    ...
    it('应该能够静音参与者', async () => {
      ...
      const muteParticipantButton = screen.getByText('静音');
      fireEvent.click(muteParticipantButton);
      
      await waitFor(() => {
        expect(mockCallManager.muteParticipant).toHaveBeenCalledWith('test-conference-id', 'participant-1', true);
      });
    });
    ...
    it('应该能够取消静音参与者', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.muteParticipant).toHaveBeenCalledWith('test-conference-id', 'participant-1', false);
      });
    });
    ...
    it('应该能够提升参与者为管理员', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.setParticipantRole).toHaveBeenCalledWith('test-conference-id', 'participant-1', 'moderator');
      });
    });
    ...
    it('应该能够降级管理员为参与者', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.setParticipantRole).toHaveBeenCalledWith('test-conference-id', 'participant-1', 'participant');
      });
    });
    ...
    it('应该能够移出参与者', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('participant-menu')).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe('权限控制', () => {
    ...
    it('应该在没有邀请权限时处理错误', async () => {
      ...
      await waitFor(() => {
        expect(defaultProps.onError).toHaveBeenCalledWith(new Error('没有邀请参与者权限'));
      });
    });
    ...
    it('应该在没有管理权限时处理参与者管理错误', async () => {
      ...
      await waitFor(() => {
        expect(defaultProps.onError).toHaveBeenCalledWith(new Error('没有管理参与者权限'));
      });
    });
    ...
    it('应该在没有结束通话权限时处理错误', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: '结束会议' })).toBeInTheDocument();
      });

      const confirmButton = screen.getByRole('button', { name: '结束会议' });
  ...
  describe('结束会议确认对话框', () => {
    it('应该显示结束会议确认对话框', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('确定要结束整个会议吗？所有参与者都将被断开连接。')).toBeInTheDocument();
    ...
    it('应该能够确认结束会议', async () => {
      ...
      await waitFor(() => {
        expect(mockCallManager.endCall).toHaveBeenCalledWith('test-conference-id', '主持人结束会议');
      });
    });
    ...
    it('应该能够取消结束会议', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('确定要结束整个会议吗？所有参与者都将被断开连接。')).not.toBeInTheDocument();
      });
    });
  });
