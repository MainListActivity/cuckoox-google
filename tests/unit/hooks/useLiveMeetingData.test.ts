...
describe('useLiveMeetings Hook', () => {
  ...
  it('should fetch initial meetings, sort them, and set up a live query', async () => {
    ...
    const meetingB = createMockMeeting('2', '2023-01-02T10:00:00Z', 'Meeting B');
    ...
    await waitFor(() => {
      expect(mockQuery).toHaveBeenCalledWith('SELECT * FROM meeting WHERE case_id = $caseId', { caseId: 'case:test' });
    });
  ...
  it('should handle UPDATE event', async () => {
    ...
    await waitFor(() => expect(result.current[0].title).toBe('Meeting A Updated'));
  });
