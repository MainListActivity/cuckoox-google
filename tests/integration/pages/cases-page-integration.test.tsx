...
describe('案件列表页面 - 完整集成测试', () => {
  ...
  describe('页面基础渲染', () => {
    it('应该正确渲染页面标题和描述', async () => {
      renderWithRealSurreal(<CaseListPage />, { authUserId: TEST_IDS.USERS.ADMIN });

      // 等待页面加载完成
      await waitFor(() => {
        // 页面可能使用国际化键或实际中文文本，先尝试常见的页面标识
        const pageTitle = screen.queryByText('案件管理') || 
      ...
      // 验证页面描述存在
      await waitFor(() => {
        const description = screen.queryByText('管理和跟踪所有破产案件的进展情况') ||
    ...
    it('应该渲染搜索框和操作按钮', async () => {
      ...
      await waitFor(() => {
        ...
        const searchInput = screen.queryByPlaceholderText('搜索案件...') ||
      ...
      await waitFor(() => {
        // 查找操作按钮，可能是不同的文本
        const exportButton = screen.queryByText('导出') || screen.queryByText(/导出/);
        const createButton = screen.queryByText('创建新案件') || 
  ...
  describe('搜索功能', () => {
    ...
    it('应该支持按案件编号搜索', async () => {
      ...
      const searchInput = screen.getByPlaceholderText('搜索案件...');
      fireEvent.change(searchInput, { target: { value: '(2024)京0108破001号' } });
  ...
  describe('案件操作功能', () => {
    it('应该为每个案件显示操作按钮', async () => {
      ...
      const actionButtons = screen.getAllByText(/查看详情|查看材料|修改状态|会议纪要/);
  ...
  describe('案件状态和程序显示', () => {
    it('应该正确显示不同的案件程序类型', async () => {
      ...
      // 验证程序类型的中文显示
      // 注意：这里的文本取决于页面的实际翻译
      const procedureElements = screen.getAllByText(/破产清算|破产重整|破产和解/);
    ...
    it('应该正确显示不同的案件阶段', async () => {
      ...
      // 验证程序阶段的显示
      // 具体文本取决于页面的实现和翻译
      const phaseElements = screen.getAllByText(/债权申报|债权人.*会议|分配|结案/);
  ...
  describe('权限控制', () => {
    ...
    it('普通用户应该只看到有权限的操作', async () => {
      renderWithRealSurreal(<CaseListPage />, { authUserId: TEST_IDS.USERS.CREDITOR_USER });
  ...
  describe('Service Worker 集成', () => {
    it('应该通过service worker正确加载数据', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('北京某某科技有限公司破产清算案')).toBeInTheDocument();
      }, { timeout: 10000 });
