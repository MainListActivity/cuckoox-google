  ...
  return (
    <div data-testid="case-list">
      ...
      {cases.length === 0 ? (
        ...
        <ul>
          {cases.map((caseItem) => (
            <li key={caseItem.id.toString()} data-testid={`case-${caseItem.id.id}`}>
              <h3>{caseItem.name}</h3>
...
// 简单的案件创建组件用于测试
const SimpleCaseCreator: React.FC<{ onCaseCreated?: () => void }> = ({ onCaseCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.case_number || !formData.case_manager_name) {
      return;
    }
    ...
    <form onSubmit={handleSubmit} data-testid="case-creator">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => setFormData(prev => ({ ...prev, case_manager_name: e.target.value }))}
        />
      </div>
      <button 
        type="submit" 
...
describe('案件管理 - 真实数据库集成测试', () => {
  ...
  describe('案件列表功能', () => {
    it('应该显示数据库中的案件列表', async () => {
      ...
      expect(screen.getByText('上海某某贸易有限公司破产重整案')).toBeInTheDocument();
      
      // 验证案件详细信息
      expect(screen.getByText('案件编号: (2024)京0108破1号')).toBeInTheDocument();
  ...
  describe('案件创建功能', () => {
    it('应该能够创建新案件', async () => {
      ...
      renderWithRealSurreal(
        <SimpleCaseCreator onCaseCreated={() => { caseCreated = true; }} />
      );
